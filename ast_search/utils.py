...
def parse_function_invocation(
    ...
    try:
        ...
        try:
            ...
            if "code" not in function_name and "child" not in function_name:
                new_arguments = [
                    x.split(".")[-1] if x.split(".")[-1] != "py" else x
                    for x in new_arguments
                ]
...
def extract_subgraph_depth_k(graph, start_node, k):
    sub_graph = nx.DiGraph()

    def build_subgraph(node, depth):
        if depth <= k:
            sub_graph.add_node(node)
            for neighbor in graph.successors(node):
                if depth + 1 <= k:
                    sub_graph.add_edge(node, neighbor)
                    build_subgraph(neighbor, depth + 1)

    build_subgraph(start_node, 0)

    return sub_graph


def get_function_summary(start_node, G: nx.Graph):
    res = ""
    for idx, (node, data) in enumerate(G.nodes(data=True)):
        summary = data.get("summary", "")
        code = data.get("code", "")
        if code == "" or node == start_node:
            continue
        res += f"[{idx}] {node}:\n```\n{code}\n```\nSummary: {summary}\n\n"
    return res


def get_calling_stacks(start_node, G: nx.Graph):
    valid_node_list = []
    for idx, (node, data) in enumerate(G.nodes(data=True)):
        code = data.get("code", "")
        if code != "":
    ...
    for idx, edge in enumerate(edge_list):
        s, e = edge[0], edge[1]
        if s in valid_node_list and e in valid_node_list:
            res += f"[{idx}]{s} -> {e} \n"
    return res

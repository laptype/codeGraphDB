...
class AstVisitor:
    ...
    def recordFunctionOverrideEdge(self, functionNameNode):
        try:
            ...
            for definition in script.goto(line=startLine, column=startColumn, follow_imports=True,
    ...
    def beginVisitName(self, node):
        ...
        for definition in self.getDefinitionsOfNode(node, self.sourceFilePath):
            ...
            try:
                ...
                print(
                    'ERROR: Encountered exception "' + e.__repr__() + '" while trying to solve the definition of node "' + node.value + '" at ' + getSourceRangeOfNode(
                        node).toString() + '.')

        if referenceIsUnsolved:
            self.client.recordReferenceToUnsolvedSymhol(self.contextStack[-1].id, srctrl.REFERENCE_USAGE,
    ...
    def beginVisitErrorLeaf(self, node):
        self.client.recordError('Unexpected token of type "' + node.token_type + '" encountered.', False,
                                getSourceRangeOfNode(node))
    ...
    def recordErrorsForUnsolvedImports(self, node):
        ...
        if node.type == 'import_from':
            ...
            if len(self.getDefinitionsOfNode(node, self.sourceFilePath)) == 0:
                self.client.recordError('Imported symbol named "' + node.value + '" has not been found.', False,
                                        getSourceRangeOfNode(node))
                return False
        return True
    ...
    def recordClassReference(self, node, definition):
        ...
        if isQualifierNode(node):
            ...
            if node.parent is not None:
                ...
                elif node.parent.type in ['arglist',
                                          'testlist'] and node.parent.parent is not None and node.parent.parent.type == 'classdef':
        ...
        for definition in self.getDefinitionsOfNode(nameNode, nodeSourceFilePath):
            ...
            if potentialSelfNode is not None:
                ...
                if potentialSelfNameNode is not None:
                    for potentialSelfDefinition in self.getDefinitionsOfNode(potentialSelfNameNode,
                                                                             definitionModulePath):
